...
class Problem:
    ...
    def __init__(self, file):
        ...
        for room in self.rooms.values():
            bayes_template.append((room.name + "_prior", '', 0.5))

        for room in self.rooms.values():
                   
            aux_str = room.name + "_prior"
            for parent in room.adj_rooms:
                aux_str += " " + parent + "_prior"  
        ...
        for sensor in self.sensors.values():
            bayes_template.append((sensor.id + "_prior", sensor.room + "_prior", {True: sensor.TPR, False: sensor.FPR}))
    ...
    def solve(self):
        ...
        while self.measure:
            ...
            aux_measure = {}
            for key in measure:
                aux_measure[key + "_prior"] = measure[key] 

            for room in self.rooms.values():
                current[room.name] = elimination_ask(room.name, aux_measure, self.network_template).prob[True]

            for room in current:
                self.network_template.variable_node(room + "_prior").cpt = {(): current[room]}
